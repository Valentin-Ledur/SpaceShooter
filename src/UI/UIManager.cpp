...
void UIManager::Init(SDL_Renderer *_renderer, int _width, int _height, int *_score_ptr, int *_last_score_ptr, int *_hp_ptr)
{
    SDL_Rect screen_rect = {0, 0, _width, _height};

    // MENU START
    TextArea title_start = TextArea(_width / 2.5, _height / 3.2, 400, 100, BLACK, "SPACE SHOOTER", nullptr, 40);
    ...
    // MENU PLAY
    TextArea _score = TextArea((int)(_width - _width * 0.05), 0, 40, 20, TRANSPARENT, "Score: ", _score_ptr, 15);
    TextArea _hp = TextArea(0, 0, 40, 20, TRANSPARENT, "HP: ", _hp_ptr, 15);
    play = Menu(_renderer, {}, {_score, _hp}, PLAY_BACKGROUND_TEXTURE_PATH, screen_rect, PLAY);

    // MENU PAUSE
    TextArea title_pause = TextArea(_width / 2.5, _height / 3.2, 300, 100, BLACK, "PAUSE", nullptr, 40);
    ...
    pause = Menu(_renderer, {btn_resume, btn_quit_pause}, {title_pause}, START_BACKGROUND_TEXTURE_PATH, {0, 0, 0, 0}, PAUSE);

    // MENU GAME OVER
    
    TextArea title_gameover = TextArea(_width / 2.5, _height / 3.2, 400, 100, BLACK, "GAME OVER", nullptr, 40);
    Button btn_restart = Button(_width / 2.5, _height * 0.48, 300, 80, DARK_GRAY, BLUE, "MENU PRINCIPAL", 30, START);
    TextArea last_score = TextArea(_width / 2.5, _height * 0.66, 300, 80, BLACK, "Score: ", _last_score_ptr, 30);
    game_over = Menu(_renderer, {btn_restart}, {title_gameover, last_score}, START_BACKGROUND_TEXTURE_PATH, screen_rect, GAME_OVER);
}
...
#else
    if (_event.key.keysym.sym == SDLK_e && _current_game_statut == PLAY)
    {
        return PAUSE;
    }
#endif
